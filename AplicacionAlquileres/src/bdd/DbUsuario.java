...
public class DbUsuario extends Conexion {
    ...
    public boolean actualizarUsuario(Usuario usuario) {
        String sql = "UPDATE usuario SET nombre=?, correo=?, telef=?, direccion=? WHERE dni=? AND rol=?";
        try (PreparedStatement stmt = conexion.prepareStatement(sql)) {
    ...
    public Usuario ver1Usuario(String dni, int rol) {
        String sql = "SELECT dni, nombre, telef, correo, direccion, contrasea FROM usuario WHERE dni = ? AND rol = ?";
        try (PreparedStatement stmt = conexion.prepareStatement(sql)) {
            stmt.setString(1, dni);
            stmt.setInt(2, rol);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
                return new Usuario(rs.getString("dni"), rs.getString("nombre"), rs.getString("telef"), rs.getString("correo"), rs.getString("direccion"), rs.getString("contrasea"), rol);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
